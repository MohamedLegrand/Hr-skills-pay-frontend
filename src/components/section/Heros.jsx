// Hero.jsx — version professionnelle avec gestion responsive
import React, {
...
const ProgressBar = memo(({ active, duration, paused }) => {
  ...
  return (
    <div style={{ height: 2, width: 20, borderRadius: 2, background: '#E5E7EB', overflow: 'hidden' }}>
      ...
      {lines.map((line, li) => (
        <span key={`${slideKey}-${li}`} style={{ display: 'block', overflow: 'hidden' }}>
          <span style={{ 
            ...
            opacity: visible ? 1 : 0, 
            transition: `transform 0.75s cubic-bezier(0.22,1,0.36,1) ${li * 90}ms, opacity 0.5s ease ${li * 90}ms` 
          }}>
            {line}
          </span>
        </span>
...
// ── Image panel responsive avec gestion d'erreur ─────────────────────────────
    ...
    <div style={{ position: 'relative', display: 'flex', justifyContent: 'center', marginTop: 20 }}>
      ...
      <div style={{ 
        ...
        <div style={{ 
          ...
          overflow: 'hidden', 
          aspectRatio: '4/3', 
          boxShadow: '0 25px 50px -12px rgba(0,0,0,0.15), 0 1px 3px rgba(0,0,0,0.05)', 
          background: '#F3F4F6' 
        }}>
          {!loaded && !imgError && (
            <div style={{ 
          ...
          {!imgError ? (
            ...
            <div style={{ 
              ...
              <div style={{ textAlign: 'center', color: 'white', padding: '16px' }}>
                <div style={{ fontSize: 'clamp(32px, 10vw, 48px)', marginBottom: '8px' }}>📷</div>
                <div style={{ fontSize: 'clamp(12px, 4vw, 14px)', fontWeight: '500' }}>{slide.badge}</div>
                <div style={{ fontSize: 'clamp(10px, 3vw, 12px)', opacity: 0.8, marginTop: '6px' }}>Image en cours de chargement</div>
              </div>
            </div>
          )}
          ...
          <div style={{ 
            ...
            width: '60%', 
            height: '50%', 
            background: `radial-gradient(ellipse at top left,${slide.accent}12 0%,transparent 70%)`, 
            pointerEvents: 'none', 
            transition: 'background 0.8s ease' 
          }} />
        </div>
        ...
        <div style={{ 
          ...
          border: `1px solid ${slide.accentBg}`, 
          transform: entered ? 'translateY(0)' : 'translateY(16px)', 
          opacity: entered ? 1 : 0, 
          transition: 'transform 0.75s cubic-bezier(0.22,1,0.36,1) 0.3s,opacity 0.6s ease 0.3s' 
        }}>
          <div style={{ 
            width: 28, 
            height: 28, 
            ...
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none"><path d="M20 6L9 17l-5-5" stroke={slide.accent} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" /></svg>
          </div>
          <div>
            <div style={{ fontSize: 11, fontWeight: 600, color: '#111827', lineHeight: 1.3 }}>{slide.badge}</div>
            <div style={{ fontSize: 9, color: '#6B7280', marginTop: 1 }}>{slide.ctaSub}</div>
          </div>
        </div>
        ...
        <div style={{ 
          ...
          transform: entered ? 'translateY(0) rotate(-2deg)' : 'translateY(-12px) rotate(-2deg)', 
          opacity: entered ? 1 : 0, 
...
const SlideContent = memo(({ slide, slideKey }) => {
  ...
  return (
    <div style={{ 
      ...
      <div style={{ 
        ...
        <button
          ...
          style={{ 
            ...
            fontWeight: 700, 
            cursor: 'pointer', 
            boxShadow: `0 2px 8px ${slide.accent}30`, 
        ...
        >
          {slide.cta}
          <svg width="12" height="12" fill="none" stroke="white" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M13 7l5 5-5 5M6 12h12" /></svg>
        </button>
        <button
      ...
      <p style={{ ...fade(340), marginTop: 8, fontSize: 10, color: '#6B7280' }}>{slide.ctaSub}</p>
      ...
      <div style={{ 
        ...
        {slide.stats.map((s, i) => (
          <div key={i}>
            <div style={{ fontFamily: "'DM Serif Display',Georgia,serif", fontSize: 'clamp(20px, 5vw, 26px)', color: slide.accent, lineHeight: 1, transition: 'color 0.5s ease' }}>{s.value}</div>
            <div style={{ fontSize: 10, color: '#6B7280', marginTop: 2, textTransform: 'uppercase', letterSpacing: '0.08em' }}>{s.label}</div>
          </div>
        ))}
      </div>
    </div>
    ...
    >
      ...
      >
        ...
        >
          ...
          <span style={{ display: 'block', color: 'rgba(255,255,255,0.9)', marginTop: 6 }}>
    ...
    <>
      ...
      >
        ...
        <div style={{ 
          ...
          width: '50%', 
          height: '100%', 
          background: `radial-gradient(ellipse at 100% 50%, ${slide.accentBg} 0%, transparent 70%)`, 
          opacity: 0.6, 
          transition: 'background 0.8s ease', 
          pointerEvents: 'none' 
        }} aria-hidden="true" />
        ...
        <div style={{ 
          ...
          {SLIDES.map((s, i) => (
            <button key={s.id} onClick={() => goTo(i)} aria-label={`Slide ${i + 1} — ${s.badge}`}
              style={{ display: 'flex', flexDirection: 'column', gap: 4, background: 'none', border: 'none', cursor: 'pointer', padding: '2px 0' }}>
              <span style={{ fontSize: 8, fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase', color: i === current ? '#111827' : '#D1D5DB', transition: 'color 0.2s' }}>
                {s.eyebrow}
              </span>
