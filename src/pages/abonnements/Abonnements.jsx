...
// ============================================================================
// COMPOSANT FAQ ITEM RESPONSIVE
// ============================================================================
const FAQItem = ({ question, answer }) => {
  const [isOpen, setIsOpen] = useState(false);

  return (
    <div className="bg-white rounded-xl sm:rounded-2xl border border-slate-200 overflow-hidden transition-all hover:border-violet-200">
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="w-full px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between gap-3 sm:gap-4 text-left hover:bg-slate-50 transition-colors"
      >
      ...
      <div className={`overflow-hidden transition-all duration-300 ${isOpen ? 'max-h-96' : 'max-h-0'}`}>
        <p className="px-4 sm:px-6 pb-3 sm:pb-4 text-xs sm:text-sm text-slate-600 leading-relaxed">{answer}</p>
      </div>
    </div>
  );
};
...
// ============================================================================
// COMPOSANT MODAL DE CONTACT RESPONSIVE
// ============================================================================
const ContactModal = ({ onClose }) => {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white rounded-2xl max-w-[90vw] sm:max-w-md w-full p-4 sm:p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-3 sm:mb-4">
          <h3 className="text-lg sm:text-xl font-bold text-slate-900">Contactez notre équipe</h3>
        ...
        <div className="space-y-3 sm:space-y-4">
          <a 
            ...
            className="flex items-center gap-2 sm:gap-3 p-2.5 sm:p-3 border border-slate-200 rounded-xl hover:border-violet-300 hover:bg-violet-50 transition-colors group"
          >
            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg bg-violet-100 flex items-center justify-center group-hover:bg-violet-200 transition-colors flex-shrink-0">
              <Mail className="w-4 h-4 sm:w-5 sm:h-5 text-violet-600" />
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-xs sm:text-sm font-medium text-slate-900 truncate">{APP_CONFIG.salesEmail}</p>
              <p className="text-[10px] sm:text-xs text-slate-500">Réponse sous 24h</p>
            </div>
            <ArrowRight className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-slate-400 group-hover:translate-x-1 transition-transform flex-shrink-0" />
          </a>
          ...
          >
            ...
            <div>
              <p className="text-xs sm:text-sm font-medium text-slate-900">{APP_CONFIG.supportPhone}</p>
              <p className="text-[10px] sm:text-xs text-slate-500">Lun-Ven, 9h-18h</p>
            </div>
            <ArrowRight className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-slate-400 ml-auto group-hover:translate-x-1 transition-transform flex-shrink-0" />
          </a>
        </div>
        
        <button
          onClick={onClose}
          className="w-full mt-4 sm:mt-6 px-4 py-2.5 sm:py-3 bg-gradient-to-r from-violet-600 to-indigo-600 text-white rounded-xl font-semibold text-sm sm:text-base hover:shadow-lg transition-all"
        >
          Fermer
        </button>
      </div>
...
const Abonnements = () => {
  ...
  return (
    <section 
      id="tarif"
      className="min-h-screen bg-gradient-to-b from-white via-slate-50/50 to-white py-12 sm:py-16 lg:py-20 px-4"
    >
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-8 sm:mb-10 lg:mb-12">
          <div className="inline-flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-4 py-1.5 sm:py-2 bg-violet-100 rounded-full mb-3 sm:mb-4">
            <Star className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-violet-600" />
            <span className="text-[10px] sm:text-xs font-semibold text-violet-700">Nos Abonnements</span>
          </div>
        ...
        <div className="flex items-center justify-center gap-2 sm:gap-4 mb-8 sm:mb-10 lg:mb-12">
          <span className={`text-xs sm:text-sm font-medium ${billingCycle === 'monthly' ? 'text-slate-900' : 'text-slate-500'}`}>
            Mensuel
          </span>
          <button
          ...
          >
            <span className={`absolute top-1 w-4 h-4 sm:w-5 sm:h-5 bg-white rounded-full transition-transform ${
              billingCycle === 'annual' ? 'left-7 sm:left-8' : 'left-1'
            }`} />
          </button>
          <span className={`text-xs sm:text-sm font-medium ${billingCycle === 'annual' ? 'text-slate-900' : 'text-slate-500'}`}>
            Annuel <span className="bg-emerald-100 text-emerald-700 text-[10px] sm:text-xs font-bold px-1.5 py-0.5 rounded-full ml-1">-2 mois offerts</span>
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 lg:gap-8 mb-12 sm:mb-16">
          {plans.map((plan) => {
            ...
            return (
              <div
                key={plan.id}
                className={`relative bg-white rounded-2xl sm:rounded-3xl border-2 overflow-hidden transition-all duration-300 hover:shadow-2xl ${
                  plan.popular
                    ? 'border-violet-500 shadow-xl shadow-violet-500/20 md:scale-105 z-10'
                    : 'border-slate-200 hover:border-violet-300 hover:shadow-lg'
                }`}
              >
                {plan.badge && (
                  <div className={`absolute top-0 left-0 right-0 py-1.5 sm:py-2 text-center text-[10px] sm:text-xs font-bold text-white ${
                ...
                <div className={`p-4 sm:p-6 pb-3 sm:pb-4 ${plan.badge ? 'pt-8 sm:pt-10' : ''}`}>
                  <h3 className="text-lg sm:text-xl font-bold text-slate-900">{plan.name}</h3>
                  <p className="text-xs sm:text-sm text-slate-500 mt-1">{plan.tagline}</p>
                  
                  <div className="mt-3 sm:mt-4 flex items-baseline gap-1 flex-wrap">
                  ...
                  <p className="text-[10px] sm:text-xs text-slate-400 mt-2">{plan.description}</p>
                </div>
                    ...
                    {plan.features.map((feature, idx) => (
                      <li key={idx} className="flex items-start gap-2 sm:gap-3">
                        {feature.included ? (
                          ...
                          <X className="w-4 h-4 sm:w-5 sm:h-5 text-slate-300 flex-shrink-0 mt-0.5" />
                        )}
                ...
                <div className="px-4 sm:px-6 pb-4 sm:pb-6 pt-1 sm:pt-2">
                  <button
                    ...
                    className={`w-full inline-flex items-center justify-center gap-1.5 sm:gap-2 px-4 sm:px-6 py-2.5 sm:py-3.5 rounded-xl font-semibold text-xs sm:text-sm transition-all duration-300 group ${
                      plan.popular
        ...
        {/* ========== GARANTIES RESPONSIVES ========== */}
        <div className="mb-12 sm:mb-16">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            {guarantees.map((item, index) => {
              const Icon = item.icon;
              return (
                <div key={index} className="flex items-center gap-2 sm:gap-3 p-3 sm:p-4 bg-white rounded-xl border border-slate-200 hover:border-violet-200 transition-colors">
        ...
        {/* ========== COMPARAISON DÉTAILLÉE RESPONSIVE ========== */}
        <div className="mb-12 sm:mb-16">
          <div className="text-center mb-6 sm:mb-8">
            <h2 className="text-xl sm:text-2xl lg:text-3xl font-bold text-slate-900 mb-2">
              Comparaison détaillée
            </h2>
            <p className="text-xs sm:text-sm text-slate-600">Visualisez toutes les fonctionnalités incluses dans chaque plan</p>
          </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[560px] sm:min-w-[640px]">
                <thead className="bg-slate-50">
                  <tr>
                    <th className="text-left p-3 sm:p-4 text-xs sm:text-sm font-semibold text-slate-700">Fonctionnalité</th>
                    <th className="text-center p-3 sm:p-4 text-xs sm:text-sm font-semibold text-slate-700">Démarrage</th>
                    <th className="text-center p-3 sm:p-4 text-xs sm:text-sm font-semibold text-white bg-gradient-to-r from-violet-600 to-indigo-600">Business</th>
                    <th className="text-center p-3 sm:p-4 text-xs sm:text-sm font-semibold text-slate-700">Enterprise</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-200">
                  {[
                    ...
                    <tr key={idx} className="hover:bg-slate-50 transition-colors">
                      <td className="p-3 sm:p-4 text-xs sm:text-sm font-medium text-slate-700">{row.feature}</td>
                      <td className="p-3 sm:p-4 text-center text-xs sm:text-sm text-slate-600">{row.basic}</td>
                      <td className="p-3 sm:p-4 text-center text-xs sm:text-sm font-medium bg-violet-50/50">{row.standard}</td>
                      <td className="p-3 sm:p-4 text-center text-xs sm:text-sm text-slate-600">{row.premium}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="mb-12 sm:mb-16">
          <div className="text-center mb-6 sm:mb-8">
            ...
            <p className="text-xs sm:text-sm text-slate-600">Tout ce que vous devez savoir sur nos abonnements</p>
          </div>
            ...
            <div className="relative">
              ...
              <div className="mt-6 sm:mt-8 flex flex-wrap justify-center gap-4 sm:gap-6 text-xs sm:text-sm text-white/70">
                <span className="flex items-center gap-1.5 sm:gap-2">
                  <Check className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-emerald-400" /> 14 jours d'essai gratuit
                </span>
                <span className="flex items-center gap-1.5 sm:gap-2">
                  <Check className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-emerald-400" /> Sans engagement
                </span>
                <span className="flex items-center gap-1.5 sm:gap-2">
                  <Check className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-emerald-400" /> Support 24/7
                </span>
                <span className="flex items-center gap-1.5 sm:gap-2">
                  <Check className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-emerald-400" /> Paiement sécurisé
                </span>
              </div>
            </div>
          </div>
